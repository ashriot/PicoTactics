...
var hp_bar_modifier: int setget set_hp_bar_modifier, \
		get_hp_bar_modifier
...
onready var _hp_bar := $Center/Offset/Sprite/HpBar as HpBar
...
func set_hp_bar_modifier(value: int) -> void:
	if _hp_bar:
		assert(not _hp_bar_animating)
		_hp_bar.modifier = value
		if _hp_bar.modifier != 0:
			_hp_bar.visible = true
		else:
			_hp_bar.visible = false


func get_hp_bar_modifier() -> int:
	var result := 0
	if _hp_bar and not Engine.editor_hint:
		result = int(_hp_bar.modifier)
	return result
...
func _animate_hpbar(change: int) -> void:
	_hp_bar_animating = true
	_hp_bar.visible = true
	_hp_bar.animate_change(change)
...
func _on_Stats_damaged(amount: int, direction: Vector2,
	...
	if get_stats().is_alive:
		...
		_animate_hpbar(-amount)
		if standard_hit_anim:
			yield(_animate_hit(direction), "completed")
		if _hp_bar_animating:
			yield(_hp_bar, "animation_finished")
	else:
		if standard_hit_anim:
