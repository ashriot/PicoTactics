...
func damage_from_attack(base_damage: int) -> int:
	...
	var defense_mod := get_stat_mod_percent(StatType.Type.DEFENSE)
	var reduced_damage := int(base_f * (1.0 - defense_mod))
...
func instant_kill(direction: Vector2, standard_hit_anim := true) -> void:
	var old_hp := hp
	hp = 0
	emit_signal("damaged", old_hp, direction, standard_hit_anim)


# Heal hp by a percentage of max_hp
func heal(heal_power: float, overflow: bool) -> void:
	var regained_hp := int(ceil(get_max_hp() * heal_power))
	if overflow:
		hp += regained_hp
	else:
		hp = int(clamp(hp + regained_hp, 0, get_max_hp()))
	emit_signal("healed", regained_hp)
